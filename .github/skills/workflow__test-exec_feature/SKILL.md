---
name: workflow__test-exec_feature
description: Laravel の Feature テストを実施し、HTTP 層の失敗原因を切り分け、結果整理と再現条件を記録するときに使う。
...
- test-specification.md に基づき Feature テストを実施するとき
...
3. 対象の Feature テスト構成と実行コマンドを確認する。
...
- Unit テスト向きの純粋ロジック確認まで Feature テストに抱え込まない。
